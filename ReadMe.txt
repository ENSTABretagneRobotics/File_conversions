...
NMEA2CSV : convert a raw NMEA data file lognav.txt to a log file compatible with lognav2KML. Before use, remove binary or invalid data (e.g. using Notepad++ replace '[^\x20-\x7E]+' with '\r\n' and then replace regex '^[^$]' with nothing). A configuration file NMEA2CSV.txt can be used to change some parameters.
