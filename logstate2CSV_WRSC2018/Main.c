...
int main(int argc, char* argv[])
{
	...
	double t_epoch2 = 0, altitude_AGL2 = 0;
	double d0 = 0;

	if (argc != 3)
	{
		strcpy(szFileInPath, "logstate.csv");
		strcpy(szFileInPath2, "logstate2.csv");
		printf("Warning : Wrong number of parameters.\n");
		printf("Usage : logstate2CSV_WRSC2018 logstate.csv logstate2.csv.\n");
		printf("Default : logstate2CSV_WRSC2018 %.255s %.255s.\n", szFileInPath, szFileInPath2);
	}
	else
	{
	...
	while (fgets3(filein, line, sizeof(line)) != NULL)
	{
			...
			&&(latitude != 0)&&(longitude != 0))
		{
			...
			while ((t_epoch2 < t_epoch)&&(fgets3(filein2, line2, sizeof(line2)) != NULL)&&
				(sscanf(line2, "%lf;%lf;%lf;%lf;%lf;%lf;%lf;%lf", &t_epoch2, &d0, &d0, &d0, &d0, &d0, &d0, &altitude_AGL2) == 8))
			{
			}
			...
			{
			case CSV_3S_WRSC_FORMAT:
				fprintf(fileout, "%d,%d,%d,%02d%02d%02d%01d%02d\n", (int)(10000000*latitude), (int)(10000000*longitude), (int)(1000.0*altitude_AGL2*cos(phihat)), hour, minute, (int)second, (int)((second-(int)second)*10.0), day);
				break;
			case CSV_2S_WRSC_FORMAT:
			default:
				fprintf(fileout, "%d,%d,%d,%02d%02d%02d%02d\n", (int)(10000000*latitude), (int)(10000000*longitude), (int)(1000.0*altitude_AGL2*cos(phihat)), hour, minute, (int)second, day);
				break;
			}
