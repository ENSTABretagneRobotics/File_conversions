...
int main(int argc, char* argv[])
{
	...
	double TimeStamp = 0, GPS_Time = 0, UTC_Time = 0, GNSS1Yaw = 0;
...
*/ 
	...
	while (fgets3(filein, line, sizeof(line)) != NULL) 
	{
		...
		{
			j++;
			GPS_Time = (double)TimeUS;
			TimeStamp = (uint32_t)(TimeUS/1000);
			//GPS_Time = GPSTOW2UNIX(GMS, GWk)/1000000.0;
			fprintf(fileout, "%f;%f;"
				"%f;%02d/%02d/%d;%d;%d;%f;0;0;"
				"%f;%f;%f;%f;%f;%f;%f;%f;%f;"
				"%f;%f;%f;"
				"%.8f;%.8f;%f;"
				"%f;%f;%f;"
				"%f;;;;;\n", 
				TimeStamp, GPS_Time, 
				UTC_Time, UTC_Day, UTC_Month, UTC_Year, UTC_Hour, UTC_Minute, UTC_Seconds, 
				AccX, AccY, AccZ, GyrX, GyrY, GyrZ, MagX/1000.0, MagY/1000.0, MagZ/1000.0, 
