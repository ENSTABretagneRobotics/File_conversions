...
inline int LoadConfig()
{
	...
	if (file != NULL)
	{
		...
		if (fgets3(file, line, sizeof(line)) == NULL) printf("Invalid configuration file.\n");
		if (sscanf(line, "%d", &CSV_format) != 1) printf("Invalid configuration file.\n");
...
int main(int argc, char* argv[])
{
	...
	if (argc != 2)
	{
		...
		printf("Usage : NMEA2CSV_WRSC2018 file.txt.\n");
		printf("Default : NMEA2CSV_WRSC2018 %.255s.\n", szFileInPath);
	}
	else
	{
	...
	while (fgets(line, sizeof(line), filein) != NULL) 
	{
			...
			(GPS_quality_indicator > 0)) 
		{
			...
			{
			case CSV_3S_WRSC_FORMAT:
				fprintf(fileout, "%02d%02d%02d%01d%02d,%d,%d\n", hour, minute, (int)second, (int)((second-(int)second)*10.0), day, (int)(10000000*latitude), (int)(10000000*longitude));
				break;
			case CSV_2S_WRSC_FORMAT:
			default:
				fprintf(fileout, "%02d%02d%02d%02d,%d,%d\n", hour, minute, (int)second, day, (int)(10000000*latitude), (int)(10000000*longitude));
				break;
			}
